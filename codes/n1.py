...
EDGE_LIST_FILE = DATASET_DIR / "dataset_WWW_friendship_new.txt"
NODE_FEATURES_FILE = DATASET_DIR / "node_features_encoded.csv"
COMMUNITY_INFO_FILE = DATASET_DIR / "community_info.pickle"
...
def generate_train_test_splits(user_graph):
    """生成训练集与测试集"""
    ...
    return graph_train, examples_train, examples_val, labels_train, labels_val, examples_test, labels_test, graph_test

# Random Walks
def compute_global_embedding(graph_train):
    """计算全局 Node2Vec 嵌入"""
    start = time.time()
    embedding = node2vec_embedding(graph_train, "Train Graph")
    global_time = time.time() - start
    print(f"Global GRL time: {global_time:.2f} seconds")
    return embedding, global_time
...
if __name__ == "__main__":
    ...
    # 生成训练和测试集
    graph_train, examples_train, examples_val, labels_train, labels_val, examples_test, labels_test, graph_test = generate_train_test_splits(user_graph)

    # 计算全局嵌入
    embedding_train, global_time = compute_global_embedding(graph_train)
    ...
    with open(COMMUNITY_INFO_FILE, "rb") as f:
        community_partition = pickle.load(f)

    embedding_train, local_time = process_major_communities(
        ig, community_partition, node_features_encoded, embedding_train
    )
